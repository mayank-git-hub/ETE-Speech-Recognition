...
class PreProcess(nn.Module):
	...
	def forward(self, data):
		...
		bin_ = torch.floor((n_fft + 1) * hz_points / rate).float()

		fbank = torch.zeros((nfilt, int(math.floor(n_fft / 2 + 1))))
		...
		for m in range(1, nfilt + 1):
			...
			fbank[m - 1, torch.arange(f_m_minus, f_m)] = (torch.arange(f_m_minus, f_m).float() - bin_[m - 1]) / (
					bin_[m] - bin_[m - 1])
			fbank[m - 1, torch.arange(f_m, f_m_plus)] = (bin_[m + 1] - torch.arange(f_m, f_m_plus).float()) / (
					bin_[m + 1] - bin_[m])

		filter_banks = torch.matmul(pow_frames, fbank.transpose(1, 0))
		filter_banks = torch.from_numpy(np.where(filter_banks == 0, np.finfo(float).eps, filter_banks))  # Numerical Stability
...
if __name__ == "__main__":
	...
	from dataloader import DataLoaderDev, my_collate
	...
	for (audio, text, token, token_id) in dev_loader:
		...
		print(preprocessor(audio[0]).shape)
